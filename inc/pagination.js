let conn = require("./db.js")

class Pagination{

    constructor(

        query,
        params = [],
        items=10

    ){

        this.query = query;
        this.params = params;
        this.items = items;
        this.currentPage = 1;

    }

    getPage(page){

        this.currentPage = page - 1;

        this.params.push(

            this.currentPage * this.items,
            this.items

        )

        return new Promise((resolve, reject)=>{

            conn.query([
                
                this.query, "SELECT FOUND_ROWS() AS FOUND_ROWS"

            ]. join(";"), this.params, (err, results)=>{

                if(err){
                    
                    reject(err);

                }else{

                    this.data = results[0];
                    this.totalReg = results[1][0].FOUND_ROWS;
                    this.totalPages = Math.ceil(this.total/this.items);
                    this.currentPage++;

                    resolve(this.data);
                    
                }

            });

        });

    }

    getTotal(){

        return this.total;

    }

    getCurrentPage(){

        return this.currentPage;

    }

    getTotalPages(){

        return this.totalPages;

    }

    getNav(params){

        let limitPagesNav = 5;
        let links = [];
        let pagStart = 0;
        let pagEnd = 0;

        if(this.getTotalPages()< limitPagesNav){

            limitPagesNav=this.getTotalPages();

        }
        

        //Primeiras páginas
        if((this.getCurrentPage() - parseInt(limitPagesNav/2)) < 1 ){

            pagStart = 1;
            pagEnd = limitPagesNav;

        }
        //Últimas páginas
        else if ((this.getCurrentPage()+parseInt(limitPagesNav/2)) > this.getTotalPages()){

            pagStart = this.getTotalPages()- limitPagesNav;
            pagEnd = this.getTotalPages();

        }else{

            pagStart = this.getCurrentPage()-parseInt(limitPagesNav/2);
            pagEnd = this.getCurrentPage()+parseInt(limitPagesNav/2);

        }

        if(this.getCurrentPage()>1){

            links.push({

                text: '<',
                href: '?' + this.getQueryString(Object.assign({}, params, {page: this.getCurrentPage()-1}))

            })
        }

        for (let i = pagStart; i <= pagEnd; i++ ){

            links.push({

                text: i,
                href: '?' + this.getQueryString(Object.assign({}, params, {page: i})),
                active: (i === this.getCurrentPage())

            })
            
        }

        if(this.getCurrentPage() < this.getTotalPages()){

            links.push({

                text: '>',
                href: '?' + this.getQueryString(Object.assign({}, params, {page: this.getCurrentPage()+1}))

            })
        }

        return links;

    }

    getQueryString(params){

        let queryString = [];

        for(let name in params){

            queryString.push(`${name}=${params[name]}`)

        }

        return queryString.join("&");

    }

}

module.exports=Pagination;